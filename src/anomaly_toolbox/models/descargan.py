...
class DeScarGANModel(k.Model):  # pylint: disable=too-many-ancestors
    ...
    def conv(
        ...
        return k.Sequential(
            ...
            + (
                [k.layers.BatchNormalization(momentum=momentum, axis=1)]
                if self._batch_norm
                else []
            )
            + [
    ...
    def deconv(
        ...
        if use_upsample:
            up_layer = k.Sequential(
                [
                    k.layers.UpSampling2D(data_format="channels_first"),
                    k.layers.Conv2D(
                        input_shape=input_shape,
                        filters=filters,
                        kernel_size=3,
...
class Generator(DeScarGANModel):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self, ill_label, n_channels=1, nf=64, batch_norm=True):
        ...
        self._c_dim = 2
        self._max_pool_fn = lambda: k.layers.MaxPool2D(data_format="channels_first")
...
class Discriminator(DeScarGANModel):  # pylint: disable=too-many-ancestors
    ...
    def call(self, inputs: List[tf.Tensor], training=False):
        ...
        pred = self._linearclass(tf.squeeze(conv, axis=[2, 3]), training=training)
        return out, pred
